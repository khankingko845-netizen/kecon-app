      ...
      <div className="px-5 pt-14">
        ...
        {notifications.length > 0 && (
          <div className="flex gap-2 mb-4">
            <button
              onClick={markAllRead}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-white text-[12px] font-semibold text-txt-secondary shadow-sm"
            >
              <Check size={12} /> Đọc hết
            </button>
            <button
              onClick={clearAll}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-white text-[12px] font-semibold text-red-400 shadow-sm"
            >
        ...
        {loading ? (
          ...
          <div className="text-center py-16">
            ...
            <p className="text-[14px] font-bold text-txt-secondary">Không có thông báo</p>
          </div>
        ) : (
            ...
            {notifications.map((n) => (
              <button
                ...
                className={`w-full text-left rounded-xl p-3.5 flex items-start gap-3 transition-all ${
                  n.read ? "bg-white opacity-70" : "bg-white shadow-sm"
                }`}
              >
                <div className={`w-10 h-10 rounded-xl ${typeColors[n.type] || "bg-gray-50"} flex items-center justify-center flex-shrink-0 mt-0.5`}>
                  {typeIcons[n.type] || <Bell size={18} />}
                </div>
                <div className="flex-1 min-w-0">
                  <p className={`text-[13px] font-bold ${n.read ? "text-txt-secondary" : "text-txt"}`}>
                    {n.title}
                  </p>
                  <p className="text-[12px] text-txt-secondary mt-0.5 line-clamp-2">{n.body}</p>
                  <p className="text-[10px] text-txt-secondary mt-1.5">{timeAgo(n.created_at)}</p>
                </div>
                {!n.read && (
